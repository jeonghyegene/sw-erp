 ...
 *
 *   1) 근무조 관리 > 근무정책 설정 (page-att-work-policy)
 *        탭: 부서별 근무정책 설정(조직도 상속) + 근무코드 설정(마스터·자동 채번 WT+D/N+일련번호)
 *        ※ 부서원 상세 편성은 「부서별 근무조 현황 > 근무조 설정」(page-att-shift)
...
(function () {
  ...
  /* 근무정책 설정 탭 — 부서별 근무정책 설정(조직도 상속) / 근무코드 설정(마스터·자동 채번) */
  const TABS = [
    { key: 'dept',  label: '부서별 근무정책 설정' },
    { key: 'shift', label: '근무코드 설정' },
  ];
  ...
  const STATE = {
    ...
    /* 전사 기본 근무정책 — 부서 신설 시 상위 조직이 없으면 상속하는 기본값('regular' 통상 | 'shift' 교대).
       전사 기본 근무코드는 App.AttShifts.globalDefault() 로 관리(근무코드 설정 탭). */
    globalPolicy: 'regular',
    /* 상세 모달에서 편집 중인 부서명 (부서별 근무코드 설정 그리드 → 상세) */
    wpEditDept: null,
    /* 근무코드 설정 탭 — 수정 중인 근무코드(있으면 그리드 대신 인-페이지 수정 화면 표시) */
    wpShiftEditCode: null,
    /* 부서별 근무코드 설정 — 부서별 구성. lazy: { deptName: { inherit, policy:'regular'|'shift', codes:[근무코드] } }
       · inherit=true → 상위 조직과 동일 기준(상속, 기본). false → 이 부서만의 별도 기준(예외).
       · policy — 근무정책(통상 'regular' / 교대 'shift'). codes — 연결된 근무코드(복수). 기본 근무코드는 deptMeta.defaultShift.
    ...
    deptCfg: {
      ...
      /* 홍보팀 — 별도·통상근무지만 사용 근무코드 미설정(예외) 예시 (계약 모달에서 경고 노출) */
      '홍보팀':       { inherit: false, policy: 'regular', codes: [] },
    },
    /* 부서 운영 메타 — 실제 부서별(파트 포함) 관리자/기본 근무코드. lazy: { deptName: { manager, defaultShift } }
       · 본부에 기본 근무코드 시드 → 하위 팀·파트가 상속(예: 경영지원본부 WTD01 / 생산본부 WTD04) */
    deptMeta: {
      '경영지원본부': { manager: '', defaultShift: 'WTD01' },
      '생산본부':     { manager: '', defaultShift: 'WTD04' },
    },
    selectedDeptId: 'C0',
  };
  ...
  function deptMeta(deptName) {
    ...
    if (!STATE.deptMeta[deptName]) STATE.deptMeta[deptName] = { manager: '', defaultShift: '' };
  ...
  function leaveRow(emp, c, editable) {
    ...
    return `<tr data-lv-row>
      ...
      <td style="font-weight:var(--fw-medium);white-space:nowrap;">${esc(emp.name)}</td>
...
<div class="modal-backdrop" id="leave-confirm-modal" data-modal-id="leave-confirm-modal" style="z-index:1300;">
  ...
  /* 근무코드를 사용 중인 부서 목록(직원 배정 기준) */
  function shiftUseDepts(code) {
    return Array.from(new Set(allEmps().filter(e => e && e.shift === code).map(e => e.dept).filter(Boolean)));
  }
  /* 근무코드 마스터 행 — 사용 중(직원 배정)이면 수정·삭제 불가. No 는 내림차순(도메인 표준).
     total/idx 로 No 부여, 전사 기본 근무코드면 비고에 '기본 근무코드' 표시(기본값 지정은 부서별 탭 「기본 설정」). */
  function shiftRow(s, no) {
    ...
    /* 사용 중인 팀 — 팀 사용지정(useDepts) + 실제 직원 배정 부서의 합집합. 한 근무코드를 여러 팀이 공통 사용. */
    const teams = Array.from(new Set([].concat(s.useDepts || [], shiftUseDepts(s.code)).filter(Boolean)));
    const deptCell = teams.length
      ? `<div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;">${teams.map(d => `<span class="pill pill--info" style="font-size:11px;white-space:nowrap;">${esc(d)}</span>`).join('')}</div>`
      : dash;
    /* 비고 — 전사 기본 근무코드면 '기본 근무코드' 표시(기본값 지정은 부서별 탭 「기본 설정」에서 관리).
       예정 변경(적용일 미도래)이 있으면 'MM/DD 변경 예정' 배지 병기 — 현재 값은 변경 전 그대로. */
    const gdef = (App.AttShifts && App.AttShifts.globalDefault) ? App.AttShifts.globalDefault() : '';
    const noteParts = [];
    if (s.isGlobalDefault || s.code === gdef) noteParts.push(`<span class="pill pill--success" style="white-space:nowrap;">기본 근무코드</span>`);
    if (s.pending && s.pending.effDate) noteParts.push(`<span class="pill pill--warning" style="white-space:nowrap;" title="적용 시작일 전까지 현재 값 유지">${esc(fmtYMD2(parseYMD(s.pending.effDate)))} 변경 예정</span>`);
    const noteCell = noteParts.length
      ? `<div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;justify-content:center;">${noteParts.join('')}</div>`
      : dash;
    /* 상태 — 사용 / 미사용 (미사용 처리된 코드는 편성에서 선택 불가). */
    const active = !(App.AttShifts && App.AttShifts.isActive) || App.AttShifts.isActive(s.code);
    const statusPill = active
      ? '<span class="pill pill--success" style="white-space:nowrap;">사용</span>'
      : '<span class="pill pill--muted" style="white-space:nowrap;">미사용</span>';
    /* 관리 — 편집 진입. 사용 중 코드는 에디터에서 출/퇴근·휴게 잠금 처리. */
    const mngCell = `<button class="btn btn--xs" type="button" data-shift-edit="${esc(s.code)}" title="편집">편집</button>`;
    return `
      <tr class="shift-tbl__row is-clickable" data-shift-card="${esc(s.code)}"${active ? '' : ' style="opacity:.62;"'}>
        ...
        <td style="text-align:center;">${mngCell}</td>
      </tr>`;
  }
  /* 분 → 'H:MM' (근무코드 마스터 표기용) */
  ...
  function renderShift() {
    ...
    if (STATE.wpShiftEditCode) {
      return `<div data-wp-shift-edit-host style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-md);padding:18px 20px;"></div>`;
    }
    const list = (App.AttShifts && App.AttShifts.list) ? App.AttShifts.list() : [];
    const right = `<button class="btn btn--sm btn--primary" type="button" data-shift-act="add">${(window.Icons && window.Icons.plus) || '+'} 근무코드 추가</button>`;
    const body = `
      ${gridBar('개', list.length, right)}
      <div class="shift-tbl-wrap" style="max-height:52vh;overflow:auto;border:1px solid var(--color-divider);border-radius:var(--radius-sm);">
        <table class="shift-tbl" style="min-width:max-content;">
          <thead>
            <tr>
              <th style="width:44px;text-align:center;">No</th>
              <th style="width:72px;">근무코드</th>
              <th style="min-width:120px;">근무코드 명</th>
              <th style="width:58px;text-align:center;">구분</th>
              <th style="width:64px;text-align:center;">출근</th>
              <th style="width:64px;text-align:center;">퇴근</th>
              <th style="width:82px;text-align:right;">총 근무시간</th>
              <th style="width:64px;text-align:right;">연장</th>
              <th style="width:64px;text-align:right;">심야</th>
              <th style="min-width:130px;text-align:center;">휴게</th>
              <th style="min-width:180px;">사용 중인 팀</th>
              <th style="width:110px;text-align:center;">비고</th>
              <th style="width:70px;text-align:center;">상태</th>
              <th style="width:64px;text-align:center;">관리</th>
            </tr>
          </thead>
          <tbody>
            ${list.length ? list.map((s, i) => shiftRow(s, list.length - i)).join('') : `<tr><td colspan="14" style="text-align:center;padding:30px;color:var(--color-text-muted);">등록된 근무코드가 없습니다.</td></tr>`}
          </tbody>
        </table>
      </div>
    `;
    return section('근무코드 설정', body) + renderCodeChangeLog();
  }

  /* ----- 근무코드 변경 이력 (전자결재 승인 내역) — 근무코드 설정 메인 화면 하단 노출 ----- */
  function renderCodeChangeLog() {
    const log = (App.AttShifts && App.AttShifts.changeLog) ? App.AttShifts.changeLog() : [];
    const typePill = (t) => {
      if (t === '추가')   return '<span class="pill pill--info">추가</span>';
      if (t === '삭제')   return '<span class="pill pill--danger">삭제</span>';
      if (t === '미사용') return '<span class="pill pill--warning">미사용</span>';
      if (t === '재개')   return '<span class="pill pill--teal">사용 재개</span>';
      return `<span class="pill pill--muted">${esc(t)}</span>`;
    };
    const rows = log.length
      ? log.map((h, i) => `
        <tr>
          <td style="text-align:center;">${log.length - i}</td>
      ...
      : `<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--color-text-muted);">변경 이력이 없습니다.</td></tr>`;
    const body = `
      <div class="shift-tbl-wrap" style="max-height:38vh;overflow:auto;border:1px solid var(--color-divider);border-radius:var(--radius-sm);">
        <table class="shift-tbl" style="min-width:max-content;">
          <thead>
            <tr>
              <th style="width:44px;text-align:center;">No</th>
              <th style="min-width:130px;text-align:center;">변경일시</th>
              <th style="width:80px;">대상 근무코드</th>
              <th style="min-width:90px;">근무코드 명</th>
    ...
    return section('근무코드 변경 이력', body, { sub: '추가 · 삭제 · 미사용 처리 — 전자결재 승인 내역' });
  }
  ...
  /* 특정 소유(source) 부서의 근무코드 기준을 그대로 적용받는 부서 목록 —
  ...
  /* 근무정책에 맞는 근무코드 후보 — 통상=주간(WTD)만 / 교대=전체(주·야) */
  function codesForPolicy(policy) {
    const all = (App.AttShifts && App.AttShifts.list) ? App.AttShifts.list() : [];
    return policy === 'shift' ? all : all.filter(s => !s.isNight);
  }

  /* ----- 부서별 근무코드 설정 — 전체 조직을 그리드로 리스트업 ----- */
  function renderDept() {
    const orgs = (HRI() && HRI().deptsOrdered) ? HRI().deptsOrdered() : [];
    const gcode = globalDefaultCode();
    const gs = gcode && App.AttShifts.get ? App.AttShifts.get(gcode) : null;
    const rowsHtml = orgs.length ? orgs.map(o => {
      ...
      return `<tr class="shift-tbl__row is-clickable" data-dept-row="${esc(name)}">
        ...
        <td style="text-align:center;"><button class="btn btn--xs" type="button" data-dept-detail="${esc(name)}">상세</button></td>
      </tr>`;
    }).join('') : `<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--color-text-muted);">조직 정보가 없습니다.</td></tr>`;

    /* 기본 근무정책·근무코드 — UI Kit .po-info(자동 채움 요약 카드). 타이틀 구분선 바로 아래 좌측 정렬(콘텐츠 폭만). */
    const defaultInfo = `
      <div class="po-info" style="min-height:34px;width:max-content;max-width:100%;margin-bottom:12px;">
        <span class="po-info__pill">
          <span class="po-info__pill-label">기본 근무정책</span>
          <span class="po-info__pill-value">${policyPill(STATE.globalPolicy || 'regular')}</span>
        </span>
        <span class="po-info__sep">|</span>
        <span class="po-info__pill">
          <span class="po-info__pill-label">기본 근무코드</span>
          <span class="po-info__pill-value">${gs ? `<span class="po-info__code">${esc(gs.code)}</span> ${esc(gs.label || '')} <span class="t-muted" style="font-weight:var(--fw-regular);">${esc(gs.start)}~${esc(gs.end)}</span>` : '<span class="t-muted" style="font-weight:var(--fw-regular);">미지정</span>'}</span>
        </span>
      </div>`;
    const globalBtn = `<button class="btn btn--sm btn--primary" type="button" data-wp-global>기본 설정</button>`;
    const body = `
      ${defaultInfo}
      ${gridBar('개', orgs.length, globalBtn)}
      <div class="shift-tbl-wrap" style="max-height:56vh;overflow:auto;border:1px solid var(--color-divider);border-radius:var(--radius-sm);">
        <table class="shift-tbl">
          <thead>
            <tr>
              <th style="min-width:180px;">부서명</th>
              <th style="width:72px;text-align:center;">부서단위</th>
              <th style="width:120px;">관리자</th>
              <th style="width:110px;text-align:center;">근무정책</th>
              <th style="min-width:150px;text-align:center;">기본 근무코드</th>
    ...
    return section('부서별 근무코드', body);
  }

  /* ============ 부서 상세 모달 본문 — 관리자·근무정책·근무코드 복수·기본코드 ============ */
  ...
  function deptModalBodyHTML() {
    ...
    if (dmeta.defaultShift && selectedCodes.indexOf(dmeta.defaultShift) < 0) dmeta.defaultShift = '';
    const effDefault = App.AttWorkPolicy.deptDefaultShift(name);
    const dis = inherit ? 'disabled' : '';

    /* ① 근무코드 기준 (동일/별도) — 상위 조직이 있을 때만. 최상위는 항상 자체 설정이라 생략. */
    ...
    /* ③ 사용 근무코드 + 기본 근무코드 — 근무코드 설정 그리드와 동일 컬럼(구분·출근·퇴근·총 근무시간·연장·심야·휴게).
       · 별도 설정 — 편집 테이블([사용] 체크 + [기본코드로 사용] 라디오)
       · 상속 — 상위에서 사용하는 근무코드만 읽기 전용 표시 */
    const cand = codesForPolicy(policy);
    /* 근태 산정 공용 컬럼 헤더 / 셀 (근무코드 설정 그리드와 동일) */
    const metaHead = `
      ...
      ? (selectedCodes.length
          ? `<div class="shift-tbl-wrap" style="overflow:auto;border:1px solid var(--color-divider);border-radius:var(--radius-sm);">
              <table class="shift-tbl" style="min-width:max-content;">
                <thead><tr>
                  <th style="width:72px;">근무코드</th>
                  <th style="min-width:120px;">근무코드명</th>
                  ${metaHead}
                </tr></thead>
                <tbody>${selectedCodes.map(inheritRow).join('')}</tbody>
              </table>
            </div>`
          : `<span class="t-muted" style="font-size:var(--fs-sm);">상위 조직에 연결된 근무코드가 없습니다.</span>`)
      : (cand.length ? `
      <div data-wpd-scroll class="shift-tbl-wrap" style="max-height:320px;overflow:auto;border:1px solid var(--color-divider);border-radius:var(--radius-sm);">
        <table class="shift-tbl" style="min-width:max-content;">
          <thead>
            <tr>
              <th style="width:72px;">근무코드</th>
              <th style="min-width:110px;">근무코드명</th>
              ${metaHead}
              <th style="width:52px;text-align:center;">사용</th>
              <th style="width:92px;text-align:center;">기본코드로 사용</th>
            </tr>
          </thead>
          <tbody>
            ${cand.map(s => {
              const sel = selectedCodes.indexOf(s.code) >= 0;
              const isDef = (inherit ? effDefault : dmeta.defaultShift) === s.code;
              return `<tr class="${sel ? 'is-selected' : ''}">
                ...
                <td style="text-align:center;"><label class="cb" style="justify-content:center;"><input type="radio" name="wpm-default" data-wpm-default="${esc(s.code)}" ${isDef ? 'checked' : ''} ${(inherit || !sel) ? 'disabled' : ''}></label></td>
              </tr>`;
            }).join('')}
          </tbody>
        </table>
      </div>`
      : `<span class="t-muted" style="font-size:var(--fs-sm);">등록된 근무코드가 없습니다. 「근무코드 설정」 탭에서 먼저 추가하세요.</span>`);
    ...
    return `
      ${wpField('부서 근무 관리자', '', managerArea, true)}
      ${parentName ? wpField('근무코드 기준', '', basisContent) : ''}
      ${wpField(`근무정책 ${REQ}`, '', policyContent)}
      ${wpField(`사용 근무코드 ${REQ}`, '', codeTable)}
      ${inherit ? '' : wpField('적용 부서', `${appliedList.length}개`, appliedContent)}`;
  }

  /* 부서 근무 관리자 지정 — 전자결재 공용 직원 선택 OffCanvas 재사용. 상세 모달의 편집 대상(STATE.wpEditDept)에 반영. */
  ...
  /* ============ 기본 설정 모달 — 전사 기본 근무정책 + 기본 근무코드 ============ */
  function ensureWpModals() {
    if (document.getElementById('wp-global-modal')) return;
    const html = `
<div class="modal-backdrop" id="wp-global-modal" data-modal-id="wp-global-modal" style="z-index:1200;">
  <div class="modal" style="width:92vw;max-width:560px;">
    <div class="modal__header">
      <div class="modal__title">기본 설정</div>
      <button class="modal__close" type="button" data-wpg-close aria-label="닫기">✕</button>
    </div>
    <div class="modal__body" data-wpg-body style="padding:18px 20px;"></div>
    <div class="modal__footer" style="display:flex;align-items:center;gap:10px;">
      <span class="t-muted" style="font-size:var(--fs-xs);margin-right:auto;line-height:1.5;">부서 신설 시 상위 조직이 없으면 이 근무정책·근무코드를 상속합니다.</span>
      <button class="btn btn--sm" type="button" data-wpg-close>취소</button>
      <button class="btn btn--primary btn--sm" type="button" data-wpg-save>저장</button>
    </div>
  </div>
</div>
<div class="modal-backdrop" id="wp-dept-modal" data-modal-id="wp-dept-modal" style="z-index:1200;">
  <div class="modal modal--lg" style="width:96vw;max-width:1080px;max-height:88vh;display:flex;flex-direction:column;">
    <div class="modal__header">
      <div class="modal__title" data-wpd-title>부서 근무코드 설정</div>
  ...
  /* ----- 기본 설정 모달 ----- */
  function renderGlobalModalBody() {
    const m = document.getElementById('wp-global-modal');
    if (!m) return;
    const codes = (App.AttShifts && App.AttShifts.list) ? App.AttShifts.list() : [];
    const gcode = globalDefaultCode();
    const pol = STATE.globalPolicy || 'regular';
    m.querySelector('[data-wpg-body]').innerHTML = `
      ${wpField('기본 근무정책', '', `
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <label class="cb cb--pill"><input type="radio" name="wpg-policy" data-wpg-policy value="regular" ${pol !== 'shift' ? 'checked' : ''}><span>통상근무</span></label>
          <label class="cb cb--pill"><input type="radio" name="wpg-policy" data-wpg-policy value="shift" ${pol === 'shift' ? 'checked' : ''}><span>교대근무</span></label>
        </div>`, true)}
      ${wpField('기본 근무코드', '', `
        <select class="select" data-wpg-code style="width:100%;max-width:360px;">
          <option value="">미지정</option>
          ${codes.map(s => `<option value="${esc(s.code)}" ${gcode === s.code ? 'selected' : ''}>${esc(s.code)} · ${esc(s.label || s.code)} (${esc(s.start)}~${esc(s.end)}${s.isNight ? ' · 야간' : ''})</option>`).join('')}
        </select>`)}`;
  }
  function openGlobalModal() {
    ensureWpModals();
    renderGlobalModalBody();
    const m = document.getElementById('wp-global-modal');
    m.classList.add('is-open');
    document.body.style.overflow = 'hidden';
  }
  function bindGlobalModal() {
    const m = document.getElementById('wp-global-modal');
    if (!m || m.dataset.bound) return;
    m.dataset.bound = '1';
    m.addEventListener('click', e => {
      if (e.target === m || e.target.closest('[data-wpg-close]')) { closeWpModal('wp-global-modal'); return; }
      if (e.target.closest('[data-wpg-save]')) {
        const pol = m.querySelector('input[name="wpg-policy"]:checked');
        const code = m.querySelector('[data-wpg-code]');
        STATE.globalPolicy = (pol && pol.value === 'shift') ? 'shift' : 'regular';
        if (code && App.AttShifts && App.AttShifts.setGlobalDefault) App.AttShifts.setGlobalDefault(code.value || '');
        closeWpModal('wp-global-modal');
        ...
        window.toast && window.toast('전사 기본 근무정책·근무코드가 저장되었습니다.', 'success');
        return;
      }
    });
  }
  /* ----- 부서 상세 모달 ----- */
  /* preserveScroll=true 면 근무코드 테이블/모달 본문 스크롤 위치를 유지(체크박스 토글 시 최상단 튐 방지) */
  ...
  function bindDeptModal() {
    ...
    m.addEventListener('click', e => {
      ...
      if (e.target.closest('[data-wpd-save]')) {
        ...
        if (!cfg.inherit || isTopDept(name)) {
          if (!(cfg.codes || []).length) { window.toast && window.toast('사용 근무코드를 하나 이상 선택해 주세요.', 'warning'); return; }
          if (!deptMeta(name).defaultShift) { window.toast && window.toast('기본 근무코드를 지정해 주세요.', 'warning'); return; }
        }
        closeWpModal('wp-dept-modal');
        const pageEl = document.getElementById('page-att-work-policy');
        if (pageEl && pageEl.dataset.wpShellMounted && STATE.wpTab === 'dept') renderWpAll(pageEl);
        window.toast && window.toast(`「${name}」 근무코드 설정이 저장되었습니다.`, 'success');
        return;
      }
    });
    /* 기준(동일/별도) · 근무정책 · 근무코드 복수 · 기본코드 */
    m.addEventListener('change', e => {
      ...
      if (code) {
        ...
        if (code.checked) set.add(code.dataset.wpmCode); else set.delete(code.dataset.wpmCode);
      ...
      /* 기본 근무코드 */
      const def = e.target.closest('[data-wpm-default]');
      if (def) { deptMeta(name).defaultShift = def.dataset.wpmDefault; renderDeptModal(true); return; }
    });
  }
  ...
  function renderWpHead() {
    return `
      <div style="padding:16px 20px 14px;">
        <div class="att-applist-tabs" style="margin-bottom:0;">
          ${TABS.map(t => `<button type="button" class="att-applist-tab ${STATE.wpTab === t.key ? 'is-active' : ''}" data-wp-tab="${t.key}">${esc(t.label)}</button>`).join('')}
        </div>
      </div>
    `;
  }
  function renderWpShell(pageEl) {
    pageEl.innerHTML = `
      <header class="att-page__head" data-wp-head></header>
  ...
  function renderWpAll(pageEl) {
    ...
    /* 근무코드 수정 화면 — 그리드 대신 host 에 인-페이지로 마운트 (앱 레이아웃 유지) */
  ...
  function bindWp(pageEl) {
    ...
    pageEl.addEventListener('click', e => {
      ...
      if (e.target.closest('[data-set-goto-shift]')) {
        if (App.Tabs && typeof App.Tabs.open === 'function') App.Tabs.open({ id: 'att-shift', label: '근무코드 설정', page: 'page-att-shift' });
        return;
      }

      /* 근무코드 마스터 — 추가 / 편집 (모달 재사용) */
      if (e.target.closest('[data-shift-act="add"]')) { App.AttShifts && App.AttShifts.openEditor && App.AttShifts.openEditor(null); return; }
      /* 근무코드 수정 — 그리드 대신 탭 안에서 인-페이지 수정 화면으로 전환 (앱 레이아웃 유지) */
      const sedit = e.target.closest('[data-shift-edit]');
      if (sedit) { e.stopPropagation(); STATE.wpShiftEditCode = sedit.dataset.shiftEdit; renderWpAll(pageEl); return; }
      const scard = e.target.closest('[data-shift-card]');
      if (scard) { STATE.wpShiftEditCode = scard.dataset.shiftCard; renderWpAll(pageEl); return; }
      if (e.target.closest('[data-set-save]')) {
        window.toast && window.toast('설정이 저장되었습니다.', 'success');
        return;
      }
      /* 부서별 근무코드 설정 — 기본 설정 모달 (전사 기본 근무정책 · 근무코드) */
      if (e.target.closest('[data-wp-global]')) { openGlobalModal(); return; }
      /* 부서별 근무코드 설정 — 상세 버튼 → 부서 상세 모달 */
      const ddet = e.target.closest('[data-dept-detail]');
      if (ddet) { e.stopPropagation(); openDeptModal(ddet.dataset.deptDetail); return; }
      /* 부서별 근무코드 설정 — 행 클릭 → 부서 상세 모달 (인터랙티브 요소·텍스트 선택 중 제외) */
  ...
  /* =========================================================
   *  공유 API — 부서별 근무정책/근무코드 (근로계약 작성·발령 등에서 참조)
   *    deptPolicy(deptName) → { regular, shift, regularSchedules, shiftSchedules, policy, codes }
   *    · 근무정책(policy)은 통상('regular') XOR 교대('shift') 택1. codes = 연결된 근무코드(복수).
  ...
  App.AttWorkPolicy = {
    ...
    /* 부서 관리자(emp id) — 미지정이면 부서장 추정값. 근무조 현황 '근무조 배치' 권한 판정에 사용. */
    ...
    deptDefaultShift(deptName) {
      ...
      return (val && used.indexOf(val) >= 0) ? val : '';
    },
    /* 근무코드가 어느 부서에든 연결(사용)되어 있는지 — 운영 중 수정 시 사유·적용일 필수 판정에 사용. */
    codeLinked(code) {
