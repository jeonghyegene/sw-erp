...
| 구분 | 수 | 비고 |
|---|---:|---|
| 전체 식별 UI | 109 | Page 33 + 업무성 하위 UI 76 |
| Page 수준 화면 | 33 | 인사 18, 근태 15 |
| 현재 메뉴 노출 Page | 24 | 인사 16, 근태 8 |
| 프로필 진입 Page | 1 | 내 정보 |
| 메뉴 숨김이나 다른 화면에서 진입하는 Page | 1 | 근무스케줄 배치 |
| 메뉴 제거·임시 숨김 Page | 7 | 직위·직책, 연차 설정, 경조사, 업무보고 3종, 근태코드 |
| 별도 업무성 Detail·Modal·OffCanvas | 76 | Detail 17, Modal 53, OffCanvas 6 |
...
| HR-COD-001 | 메뉴 제거: 공통 코드로 이관 | 직위·직책 설정 | Page | 없음 | `#page-hr-position` 직접 DOM | `page-hr-position.js` | 직위·직책 코드와 사용 여부·순서 관리 | 기초 설정 | 조회, 등록, 수정, 순서 변경, 사용 중지 | 확정 | `nav-data.js` 제거 주석, `initPage()` |
...
| ATT-SSA-001 | 다른 화면에서 진입 | 근무스케줄 배치 | Page | ATT-SSV-001 | `att-shift → #page-att-shift`, 메뉴 `hidden:true` | `page-att-shift.js` | 부서원 일별 근무조 배정표 작성·제출 | 근무스케줄 | 주·월 조회, 일별 변경, 일괄 변경, 월 복사, 제출, 변경 이력 | 확정 | `nav-data.js` hidden, 파일 헤더 |
...
| ATT-LPS-001 | 임시 숨김 | 연차 설정 | Page | 없음 | `#page-att-leave-set`, 메뉴 주석 | `page-att-settings.js` | 연차 부여·가산·이월·소멸 정책 설정과 발생 미리보기 | 기초 설정 | 정책 편집, 미리보기, 적용일 지정, 확정 | 확정 | `nav-data.js` 임시 숨김 주석, `initLeaveSetPage()` |
| ATT-EVT-001 | 임시 숨김 | 경조사 현황 | Page | 없음 | `#page-att-event`, 메뉴 블록 주석 | `page-att-event.js` | 승인 완료 경조사 신청의 인사총무 처리 큐 | 경조사 운영 | 조회, 결재문서 확인, 경조휴가·경조금·화환 처리·취소 | 확정 | `nav-data.js` 숨김 블록, 파일 헤더 |
| ATT-WRM-001 | 임시 숨김 | 주간 업무보고 작성 | Page | 없음 | `#page-att-report-my`, 메뉴 블록 주석 | `page-att-report-my.js` | 부서 업무분류별 주간 업무보고 작성 | 업무보고 | 주 이동, 입력, 임시저장, 제출 | 확정 | `nav-data.js` 숨김 블록, 파일 헤더 |
| ATT-WRS-001 | 임시 숨김 | 주간 업무보고 현황 | Page | 없음 | `#page-att-report-status`, 메뉴 블록 주석 | `page-att-report-status.js` | 부서·구성원별 주간 업무보고와 제출률 조회 | 업무보고 | 주·부서·구성원 전환, 제출 상태·내용 조회 | 확정 | `nav-data.js` 숨김 블록, 파일 헤더 |
| ATT-WRC-001 | 임시 숨김 | 업무보고 설정 | Page | 없음 | `#page-att-wr-settings`, 메뉴 블록 주석 | `page-att-wr-settings.js` | 부서별 업무 분류 양식 관리 | 업무보고 기초 설정 | 부서 선택, 분류 추가·수정·삭제·저장 | 확정 | `nav-data.js` 숨김 블록, `App.WorkReport` |
| ATT-COD-001 | 메뉴 제거: 시스템 관리로 이관 | 근태코드 설정 | Page | 없음 | `#page-att-code` 직접 DOM | `page-att-code.js` | 근태·휴가 3단계 사유 코드와 연차 차감 여부 관리 | 기초 설정 | 조회, 중·소분류 등록·수정, 순서 변경, 사용 여부 관리 | 확정 | `nav-data.js` 제거 주석, 파일 헤더 |
...
| HR-EMP-001-D01 | HR-EMP-001 | 인사카드 상세 | OffCanvas | `oc-hr-card` | 권한별 인사정보·계약·이력 조회 | 확정 | `index.html`, `page-hr-employee.js` |
| HR-EMP-001-M01 | HR-EMP-001 | 인사정보 수정 요청 | Modal | `modal-hr-edit` | 수정 항목·변경값·사유·증빙 제출 | 확정 | `index.html` |
| HR-EMP-001-M02 | HR-EMP-001 | 인사카드 PDF 출력 | Modal | `modal-hr-pdf` | 출력 섹션 선택, 인쇄·PDF | 확정 | `index.html` |
| HR-EMP-001-M03 | HR-EMP-001 | 부서 관리 | Modal | `modal-emp-dept-manage` | 부서 추가·수정·사용 중지·구조 관리 | 확정 | `index.html`, `App.HrDeptManage` |
| HR-EMP-001-M04 | HR-EMP-001 | 사원 부서 이동 | Modal | `modal-org-move` | 대상 사원의 소속 이동과 조직 변경 승인 요청 | 확정 | `index.html`, `page-hr-employee.js` |
| HR-EMP-002-M01 | HR-EMP-002 | 임직원 개별 등록 | Modal | 페이지 생성 Modal | 입사 기본정보 등록, 계정·계약 단계 시작 | 확정 | `bindCreateModal()`, `openCreateModal()` |
| HR-EMP-002-M02 | HR-EMP-002 | 임직원 일괄 등록 | Modal | `modal-empi-bulk` | 업로드 파일 검증·대상 일괄 등록 | 확정 | `bindBulkModal()`, `openBulkModal()` |
| HR-EMP-002-O01 | HR-EMP-002 | 입사자 상세 패널 | OffCanvas | `empi-detail-pane` | 입사 마일스톤·계약·서류·계정 상태 관리 | 확정 | `renderDetailPane()` 계열 |
| HR-EMP-002-O02 | HR-EMP-002 | 인사정보카드 | OffCanvas | `oc-empi-card` | 공개·비공개·계약·입사서류 탭 조회·편집 | 확정 | `openDrawer()`, `renderDrawer()` |
...
| HR-CTR-001-M01 | HR-CTR-001 | 계약서 일괄 작성 | Modal | `modal-ctr-bulk` | 조직·직원 선택, 공통·개별 조건 편집, 일괄 생성 | 확정 | `openBulkModal()` |
...
| HR-PAY-001-M06 | HR-PAY-001 | 지급항목 편집 | Modal | `modal-pi-editor`, `modal-prs-additem` | 지급항목 추가·정책 편집·정산 회차 반영 | 확정 | `hr-payitem-data.js`, `openAddItemModal()` |
...
| HR-RSG-001-M01 | HR-RSG-001 | 퇴사 처리 | Modal | `modal-lv-resign` | 대상자·퇴사일·사유·연차정산·계정회수·자산반납 처리 | 확정 | `commitResign()` |
...
| HR-COD-001-M01 | HR-COD-001 | 직위·직책 코드 편집 | Modal | 페이지 생성 Modal | 코드명·ID·설명·사용 여부 등록·수정 | 확정 | `openModal(kind, editId)` |
...
| ATT-STS-001-M04 | ATT-STS-001 | 신청 현황 | Modal | `modal-att-applist` | 연차·초과근무·근태 신청과 승인 상태 조회 | 확정 | `openAppListModal()` |
...
| ATT-WPL-001-M02 | ATT-WPL-001 | 근무조 추가·수정 | Modal | `modal-shift-editor` | 시간·휴게·사용부서·상태 등록·수정 | 확정 | `App.AttShifts.openEditor()` |
...
| ATT-SSA-001-M01 | ATT-SSA-001 | 근무조 일괄 변경 | Modal | `modal-shift-bulk` | 선택 직원·기간의 근무조 일괄 변경 | 확정 | `applyBulk()` |
| ATT-SSA-001-M02 | ATT-SSA-001 | 근무조 월 복사 | Modal | `modal-shift-copy` | 원본월을 대상월로 복사 | 확정 | `applyCopyMonth()` |
| ATT-SSA-001-M03 | ATT-SSA-001 | 근무조 마스터 | Modal | `modal-shift-master` | 근무조 목록·사용부서 조회와 편집 진입 | 확정 | `openMasterModal()` |
...
| ATT-LPS-001-M01 | ATT-LPS-001 | 연차 발생 미리보기 | Modal | `leave-policy-modal` | 적용 정책 기준 전 직원 발생량 검증·수동 조정 | 확정 | `openLeaveMode()` |
| ATT-EVT-001-M01 | ATT-EVT-001 | 경조사 업무 처리 | Modal | `modal-evt-proc` | 경조휴가·경조금·화환 처리 완료·취소 | 확정 | `openProcModal()` |
| ATT-COD-001-M01 | ATT-COD-001 | 근태코드 편집 | Modal | 페이지 생성 Modal | 중·소분류 코드와 연차 차감 여부 등록·수정 | 확정 | `openModal(depth, editId)` |

## 5. 메뉴 노출 상태 해석

- `확정`은 화면 존재와 프론트엔드 동작이 소스에서 확인됐다는 뜻이며, 실제 백엔드 저장·권한 검증까지 확정한다는 뜻이 아니다.
- `ATT-SSA-001`은 메뉴에서 숨겨졌지만 `ATT-SSV-001`의 액션으로 진입하므로 운영 흐름에 포함한다.
- `HR-COD-001`, `ATT-LPS-001`, `ATT-EVT-001`, `ATT-WRM-001`, `ATT-WRS-001`, `ATT-WRC-001`, `ATT-COD-001`은 DOM과 초기화 코드가 남아 있으나 메뉴는 제거 또는 임시 숨김이다.
